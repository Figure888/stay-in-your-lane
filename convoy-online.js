...
(function () {
  ...
  function startPolling() {
    ...
    showTable();
    refresh();
    // Chat rides along with the game, so it appears and disappears with it.
    if (window.Social && gameId) {
      window.Social.attachChat(gameId, (state && state.oppName) || 'Opponent');
    }
  ...
  function stopPolling() {
    ...
    if (window.Social) window.Social.detachChat();
  }
